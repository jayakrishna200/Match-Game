...
class App extends Component {
  state = {
    ...
    isTimeRunning: true,
    timeInSeconds: 60,
    activeThumbnailImg: imagesList[0].imageUrl,
    activeThumbnailId: imagesList[0].id,
  }

  renderProjectsList = getProjectId => {
    return (
      <ul>
        <div>
          <TabItem
  ...
  //   getThumbnailId = id => {
  //     console.log('getThumbnail() is triggerd', id)
  //     activeThumbnailId = [...activeThumbnailId, id]
  //     console.log('activeThumbnailId', activeThumbnailId)
  //   }
  getThumbnailImgUrl = thumbnailItem => {
    const thumbnailImgUrl = thumbnailItem.imageUrl
  ...
  getThumbnailItem = () => {
    const randomIndex = Math.floor(Math.random() * imagesList.length) // Corrected random index calculation
    const thumbnailItem = imagesList[randomIndex]
    return thumbnailItem
  }

  setNewThumbnailIdAndImage = () => {
    let thumbnailItem = this.getThumbnailItem()
    console.log('setting new thumbnailId', thumbnailItem.id)
    this.setState({
      activeThumbnailImg: thumbnailItem.imageUrl,
      activeThumbnailId: thumbnailItem.id,
    })
  }

  checkIsImageMatched = projectId => {
    const {activeThumbnailId} = this.state
    if (projectId === activeThumbnailId) {
      this.setNewThumbnailIdAndImage()
      this.setState(prevState => ({score: prevState.score + 1}))
      console.log('Image Matched')
    } else {
      this.setState(prevState => ({isTimeRunning: false, timeInSeconds: 0}))
  ...
  playAgain = () => {
    ...
    this.setState(prevState => ({
      ...
      timeInSeconds: 60,
      activeThumbnailId: imagesList[0].id,
      activeThumbnailImg: imagesList[0].thumbnailUrl,
    }))
  ...
  getProjectId = projectId => {
    ...
    console.log('projectId', projectId, 'activeThumbnailId', activeThumbnailId)
    this.checkIsImageMatched(projectId)
  }
  ...
  renderThumbnailItem = () => {
    ...
    return (
      <ul>
        <div>
          <ThumbnailItem activeThumbnailImg={activeThumbnailImg} />
        </div>
      </ul>
    )
  }
  ...
  renderHeaderItem = () => {
    const {score, timeInSeconds, isTimeRunning} = this.state
        ...
        <nav>
          <div className="header-item">
            <img
              ...
              alt="website logo"
            />
            <div className="score-and-time-container">
              <p className="score">
  ...
  tick = () => {
    let {timeInSeconds, isTimeRunning} = this.state
    if (timeInSeconds === 0 || isTimeRunning === false) {
      this.clearTimeInterval()
      this.setState({timeInSeconds: 0, isTimeRunning: false})
    } else {
  ...
  render() {
    const {score, isTimeRunning, activeThumbnailId, activeTabId} = this.state
    console.log(score)
    return (
      <>
        {this.renderHeaderItem()}
        <div className="main-bg">
          {isTimeRunning && this.renderThumbnailItem()}

          {isTimeRunning
            ? this.renderProjectsList(this.getProjectId)
